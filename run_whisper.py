...
from audiomentations import Compose, AddBackgroundNoise, AddGaussianNoise, PolarityInversion
...
def main():
    ...
    def prepare_dataset(batch):
        sampling_rate = batch[audio_column_name]["sampling_rate"]

        # apply augmentation
        audio = batch[audio_column_name]["array"]
        augmented_audio = augment_waveform(samples=audio, sample_rate=sampling_rate)
        batch[audio_column_name]["array"] = augmented_audio

        # process audio
        sample = batch[audio_column_name]
        inputs = feature_extractor(
            sample["array"], sampling_rate=sampling_rate, return_attention_mask=forward_attention_mask
        )
        ...
        # process targets
        input_str = remove_punctuations_and_spaces(batch[text_column_name]) if do_lower_case else batch[text_column_name]
    ...
    def compute_metrics(pred):
        ...
        pred_str = list(map(remove_punctuations_and_spaces, pred_str))
        label_str = list(map(remove_punctuations_and_spaces, label_str))
